...
import pickle
from data.transforms import get_tumor_transforms, tumor_transfo
import pandas as pd
...
class Liver_normalization_dataset(torch.utils.data.Dataset):
    def __init__(self, summery_path: str = 'data/summery_new.txt', mode: str = 'all_model'):
        ...
        # use vision_feature directly
        df = pd.read_csv("data/vision_features.csv")
        self.vf_dict = df.set_index('uid').T.to_dict('list')
        ...
        for item in summery:
            ...
            if self.mode == 'fusion' or 'two_model' or 'bert':
    ...
    def __getitem__(self, index):
        ...
        vf = np.array(self.vf_dict[int(uid)])
        vf = torch.Tensor(vf)
        ...
        if self.mode == 'img':
            ...
            item = {'image':vision_tensor}
            transform= get_tumor_transforms(roi_size =(64, 512, 512))
            item = transform(item)
            return item
        elif self.mode == 'radio':
            return radio_tensor, label_tensor
            ...
            return radio_tensor, tumor_transfo(vision_tensor), label_tensor
        elif self.mode == 'radio_img_label_test':
            return uid, radio_tensor, vision_tensor, label_tensor
        ...
        elif self.mode == 'new_data':
            return clinical_tensor, radio_tensor, vf, label_tensor
        elif self.mode == 'new':
            return radio_tensor, vf, label_tensor
        elif self.mode == 'vf':
            return vf, label_tensor
        elif self.mode == 'uid':
            return uid
        else:
            return None
