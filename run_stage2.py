...
def prepare_to_train(model_index, seed, device, fold, data_parallel):
    ...
    patient_info = get_pinfo(cli_file ="./data/summery_new.xlsx", vision_file = "./data/vision_seed42/vision_features_{}.csv".format(fold))
    ...
    train_uids = [dataset[i] for i in train_index]
    test_uids = [dataset[i] for i in test_index]

    train_indices = np.where(np.isin(patient_info['uid'], train_uids))[0]
    test_indices = np.where(np.isin(patient_info['uid'], test_uids))[0]

    # _, g = graph_bulider(patient_info)
    x, edge_index, edge_attr = adj_matrix(patient_info, mode = experiment_settings['Mode'])
    ...
    if 'Input_args' in experiment_settings:
        input_args = experiment_settings['Input_args']
        model = _model(**input_args)
    else:
        model = _model()
    ...
    if 'w1' in experiment_settings:
        criterion = experiment_settings['Loss'](w1 = experiment_settings['w1'], w2 = experiment_settings['w2'], w3 = experiment_settings['w3'])
    else:
    ...
    _run(observer, patient_info, data, train_indices, test_indices, experiment_settings['Epoch'], model, device, optimizer, criterion)
    end_time = time.time()
...
if __name__ == "__main__":
    ...
    print(args)
    # for i in range(5):
    #     prepare_to_train(model_index=args.model, seed=args.seed, device=args.device, fold= i , data_parallel=args.data_parallel)
